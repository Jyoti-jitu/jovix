...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-[#e6e7e2]/95 backdrop-blur-md border-b border-stone-300/80 transition-all">
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-stone-300/80 bg-[#e6e7e2] px-6 py-6 flex flex-col gap-4 text-base font-medium text-slate-800 shadow-xl animate-in slide-in-from-top-2 duration-200">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
